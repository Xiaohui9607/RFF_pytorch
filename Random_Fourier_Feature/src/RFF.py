...
class RFF:
	...
	def __call__(self, x, dtype=torch.FloatTensor):
		self.initialize_RFF(x, self.sigma, True, dtype)
...
if __name__ == "__main__":
	...
	rff_torch = RFF(30000)	
	rbf_torch = rff_torch.get_rbf(torch.from_numpy(X).type(torch.FloatTensor), sigma, True)


	rff_torch_inner = RFF(30000)	
	psi = rff_torch_inner(torch.from_numpy(X).type(torch.FloatTensor), sigma)
	rbf_torch_inner = torch.matmul(psi, psi.T)
